...
## Unlimited worker defaults (September 12, 2026)

- All 40 automated tests passed. Five new tests cover unlimited defaults for both workers, omitted Claude turn flags, `null` limit metadata, independent explicit limits, values above the former caps, and invalid arguments. Existing timeout termination and cancellation tests still pass. The new cases reproduced the old defaults and caps as failures before the fix.
- Direct smoke tests through `~/.local/bin/claudex` completed with both fake workers and confirmed `timeout_seconds: null`, `max_turns: null`, and no forwarded turn cap. The installed command and both lead skills resolve to this checkout. The existing global Codex instruction block was refreshed and its unrelated content verified unchanged.
- Both repository and installed-command doctor checks reported `ready`: Claude Code 2.1.270 with native `claude.ai` Max login; Codex CLI 0.154.0 with ChatGPT login. These checks do not send inference requests.
- A native Claude review invoked through `~/.local/bin/claudex` without limit flags completed in 88.25 seconds and 10 turns. Its result records both limits as `null`, no permission denials, and no correctness findings. One README wording inconsistency was corrected. Artifacts: `/home/astrosteveo/.local/state/claudex/runs/20260912-232734-f2e7d40674`. The source-change warning lists the README and Claude-lead skill edits made by Codex during the review. No new native Codex inference test was run for this change.

Agent guidance now requires user-requested limits and distinguishes polling from execution deadlines. Unlimited defaults apply to inference workers; CLI health checks and local Git operations retain diagnostic timeouts.
